...
var lst: array[1..21] of boolean; i: integer; x: char; fl: boolean; w: set of char; lst2:array[1..21] of char;
begin
   lst2[1]:= 'b';
   lst2[2]:= 'c';
   lst2[3]:= 'd';
   ...
   repeat
     ...
     if x<>'.' then
     fl:=true;
     if ((ord(x) < 97) or (ord(x) > 122)) and (ord(x) <> 32) and (ord(x) <> 46) then begin write('Error'); exit; end;
   ...
   for i:=1 to 21 do
   if lst[i]=true then write(lst2[i]);
end.
